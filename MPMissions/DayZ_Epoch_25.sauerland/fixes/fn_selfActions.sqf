...
if (!isNull cursorTarget && !_inVehicle && !_isPZombie && (player distance cursorTarget < _allowedDistance) && _canDo) then {	//Has some kind of target
	...
	_player_lockUnlock_crtl = false;
	 ...
	 if (_canDo && (speed player <= 1) && (_cursorTarget isKindOf "Plastic_Pole_EP1_DZ")) then {
			...
			_adminList = ["76561198009425402","76561198018007006","76561198121993182","76561198089990137","76561198088732522" ,"76561198161513402"]; // Add admins here if you admins to able to manage all plotpoles
		...
		if (s_player_plot_boundary_on < 0) then {
			If (_PlotsmarkersNear == 0 ) then{
				s_player_plot_boundary_on = player addAction ["Show plot boundary", "Scripts\A_Plot_for_Life\Action\object_showPlotRadius.sqf", "", 1, false];
			};
		 };	
		 if (s_player_plot_boundary_off < 0) then {
			If (_PlotsmarkersNear > 0 ) then{
				s_player_plot_boundary_off = player addAction ["Remove plot boundary", "Scripts\A_Plot_for_Life\Action\object_removePlotRadius.sqf", "", 1, false];
			};
		};
	...
	if(_player_deleteBuild) then {
		if (s_player_deleteBuild < 0) then {
			s_player_deleteBuild = player addAction [format[localize "str_actions_delete",_text], "Scripts\A_Plot_for_Life\Action\remove.sqf",_cursorTarget, 1, true, true, "", ""];
		};
	} else {
	...
	if(_isTent && (player distance _cursorTarget < 3)) then {
		if (_ownerID == _playerUID) then {
			...
			if(("ItemJerrycan" in _magazinesPlayer) && ("ItemMatchbox_DZE" in weapons player)) then {
				if (s_player_packtent < 0) then {
					s_player_packtent = player addAction [localize "STR_EPOCH_ACTIONS_DESTROYTENT", "Scripts\A_Plot_for_Life\Action\remove.sqf",_cursorTarget, 1, true, true, "", ""];
				};
			};
		};
	} else {
...
// add the following
	...
	if ((_cursorTarget isKindOf "ModularItems") || (_cursorTarget isKindOf "Land_DZE_WoodDoor_Base") || (_cursorTarget isKindOf "CinderWallDoor_DZ_Base")) then {
		...
		if (s_player_upgrade_build < 0) then {
			...
			s_player_upgrade_build = player addAction [format[localize "STR_EPOCH_ACTIONS_UPGRADE",_text], "Scripts\A_Plot_for_Life\Action\player_upgrade.sqf",_cursorTarget, -1, false, true, "",""];
		};
	} else {
		player removeAction s_player_upgrade_build;
		s_player_upgrade_build = -1;
	...
	if((_isDestructable || _cursorTarget isKindOf "Land_DZE_WoodDoorLocked_Base" || _cursorTarget isKindOf "CinderWallDoorLocked_DZ_Base") && (DZE_Lock_Door == _characterID)) then {
		...
		if (s_player_downgrade_build < 0) then {
			...
			s_player_downgrade_build = player addAction [format[localize "STR_EPOCH_ACTIONS_REMLOCK",_text], "Scripts\A_Plot_for_Life\Action\player_buildingDowngrade.sqf",_cursorTarget, -2, false, true, "",""];
		};
	} else {
		player removeAction s_player_downgrade_build;
		s_player_downgrade_build = -1;
	...
	*/
	...
	if (_typeOfCursorTarget in ["MMT_Civ","Old_moto_TK_Civ_EP1","CSJ_GyroC","BAF_GPMG_Minitripod_W"]) then {

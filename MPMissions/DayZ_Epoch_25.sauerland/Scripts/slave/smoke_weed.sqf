...
if (vehicle player != player) exitWith {cutText [format["Dude! No smoking in a vehicle!"], "PLAIN DOWN"];};
cutText ["Let's smoke this shit!", "PLAIN DOWN"];
...
while {true} do {
	...
	nonapsi_ef ppEffectCommit 1;
	uiSleep random(1);
	if (_time + 50 < time) exitWith {[_effects] call Remove_Drug_effects;};
};

private ["_display","_body","_playerID","_array","_source","_method","_canHitFree","_isBandit","_punishment","_humanityHit","_myKills","_myKillsB","_humanity","_kills","_killsV","_myGroup"];
...
//};
if (_body hasWeapon "ItemMap") then {_body removeWeapon "ItemMap";};
	...
	if ((!isNull _source) && (_source != player)) then {
		...
		if (!_punishment) then {
			...
			if (round(random(3)) == 1) then {
				PVDZE_send = [_source,"XPkill",[1,1,1]];
				publicVariableServer "PVDZE_send";
			};
			*/
		} else {
			...
			*/
			...
			/*
			// Give humanity from bandit kills?
...
//Custom deathScreen
//100 cutRsc ["DeathScr","BLACK OUT",1];

playMusic "dayz_track_death_1";
//playSound "deathscreen"; // and remove all "fadeSound" lines

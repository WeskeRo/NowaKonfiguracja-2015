...
if (!UK_transfuse) exitWith { titleText ["You unlock this ability on XP level 5","PLAIN"]; };
...
_bloodbagLastUsedTime = 1; // Amount of time in seconds before player can use self bloodbag again after a succesful use
_infectionChance = 20; // Percent chance of player infection on self bloodbag (10 = 10% | 2 = 50% | 1 = 100%)
...
_humanityAmount = 25; // Amount of humanity to give player if _humanityBool is true (250 is default for normal bloodbags)
...
if(_bloodbagTime < _bloodbagLastUsedTime) exitWith { // If cool down is not done then exit script
